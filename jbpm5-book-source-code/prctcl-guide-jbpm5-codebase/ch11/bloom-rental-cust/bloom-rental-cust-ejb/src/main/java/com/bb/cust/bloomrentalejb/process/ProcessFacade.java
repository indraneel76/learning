...
public class ProcessFacade implements ProcessFacadeLocal {
	...
	public void startBPM(String processName, Map<String, Object> processData,
		...
		try {
			...
			processData.put("sessionId", ksession.getId());
			logger = new JPAWorkingMemoryDbLogger(ksession);
			ksession.getWorkItemManager().registerWorkItemHandler(
					"Service Task", new ServiceTaskHandler());
			ksession.getWorkItemManager().registerWorkItemHandler("Human Task",	HumanTaskHelper.getBloomJMSWorkItemHandler(ksession));
